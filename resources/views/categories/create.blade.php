...
--}}
        ...
        <form method="POST" action="{{ route('categories.store') }}" class="mt-5 max-w-sm mx-auto border-white">
            @csrf
            <h2 class="font-bold text-white">Create a new Category</h2>
            <br>
            <div>

                    <label for="type" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Category Type:</label>
                    <select id="type" name="type" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
                        ...
                        <option value="{{$parent_category_item->name}}">{{$parent_category_item->name}}</option>
                        @endforeach
                    </select>
